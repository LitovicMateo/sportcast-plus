...
const ArticleListItem: React.FC<ArticleListItemProps> = ({ post }) => {
  ...
  return (
    <div className={`
      ...
      `}>
      <ArticleImage slug={post.slug} url={post.featuredImage.node.sourceUrl} />
      <ArticleMetadata author={authorName} date={date} category={categoryName} content={post.excerpt} title={post.title} />
    </div>
  );
};
